...
describe('DxButton snapshot testing', () => {
  ...
  function renderButtonVariants() {
    return html`
      <div data-testid="dx-button-layout" style="display: flex; flex-direction: column; gap: 20px; margin: 20px;">
        <div style="display: flex; justify-content: space-between;">
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin-bottom: 10px;">Button Default</label>
            <dx-button buttontext="Button" variant=${BUTTON_VARIANT.BUTTON_TEXT_VAR} exportparts="${Object.values(BUTTON_PARTS).join(',')}">
            </dx-button>
          </div>
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
  ...
  it('DxButton - should capture snapshot for all possible variants and types of AC button component for Authoring styling', async () => {
    const link = appendEnchantedStylingLink();

    render(renderButtonVariants(), document.body);
    await browser.checkElement(await $('>>>div[data-testid="dx-button-layout"]'), 'dx-button-snapshot-baseline-authoring');
    document.head.removeChild(link);
  });

  it('DxButton - should capture snapshot for inverse variants and types of AC button component for Authoring styling', async () => {
    const link = appendEnchantedStylingLink();

    render(renderButtonInverseVariants(), document.body);
    await browser.checkElement(await $('>>>div[data-testid="dx-button-layout"]'), 'dx-button-inverse-snapshot-baseline-authoring');
    document.head.removeChild(link);
  });
});
